...
# Main App Header
st.markdown(f'<div class="main-header">UPSC CSE {selected_subject} Answer Evaluator</div>', unsafe_allow_html=True)
st.markdown(f'<div class="sub-header">Evaluate handwritten answer scripts for {selected_subject} with Vision OCR, solve tricky doubts, and build a reusable Strategy Vault.</div>', unsafe_allow_html=True)
...
# ==========================================
with tab1:
    st.header(f"📝 Handwritten {selected_subject} Answer Script Evaluator")
    ...
    with col_up2:
        max_marks = st.selectbox("Maximum Marks", options=[10, 15, 20, 125, 250], index=1)
        st.info(f"💡 **Evaluation Rubric ({selected_subject})**: Assesses Demand of Question, Subject Rigor, Structural Balance, Value Addition (Articles/Data/Diagrams), and Scored Marks.")
...
with tab2:
    ...
    st.markdown(f"Ask direct questions or previous year questions (PYQs) for {selected_subject} for which you cannot find standard model answers.")
    ...
    with col_q2:
        paper_scope = st.selectbox("Target Paper Scope", options=["Sociology Optional", "GS 1", "GS 2", "GS 3", "GS 4 (Ethics)", "Essay Evaluator"], index=0)
...
with tab3:
    ...
    st.markdown(f"Upload topper answer copies (PDFs) to automatically extract reusable definitions, quotes, diagrams, and intro/outro templates into your **Strategy Vault**.")
    ...
    st.markdown("---")
    st.subheader(f"📖 Browse Complete {selected_subject} Strategy Vault")
    
    vault_view = load_vault()
    vault_tab1, vault_tab2, vault_tab3, vault_tab4 = st.tabs(["Definitions & Articles", "Quotes & Judgments", "Intro/Outro Templates", "Diagram Schematics"])
